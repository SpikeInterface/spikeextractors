...
class MaxTwoRecordingExtractor(RecordingExtractor):
    ...
    is_writable = False
    mode = 'file'
    installation_mesg = "To use the MaxTwoRecordingExtractor install h5py: \n\n pip install h5py\n\n"  # error message when not installed

    def __init__(self, file_path, well_number=0, rec_number=0):
        assert HAVE_MAX, self.installation_mesg
        RecordingExtractor.__init__(self)
        self._file_path = file_path
    ...
    def _initialize(self):
        self._filehandle = h5py.File(self._file_path, 'r')
        well_name = f'well00{self.well_number}'
        rec_name = f'rec000{self.rec_number}'
